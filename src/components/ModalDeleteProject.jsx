...
import "./ModalDeleteProject.css";

const ModalDeleteProject = ({ closeModal, update, estado }) => {
  ...
  const deleteData = () => {
    setIsLoading(true);
    fetch("https://srcaapi.gabrielangeles.com/removeProject", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({ _id: estado.data }),
    }).then(
      (data) => {
        if (data.status === 401) {
          setUser({ type: "logout" });
